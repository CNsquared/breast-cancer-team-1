...
def parse_args():
    ...
    parser.add_argument('--hidden_dim', type=int, nargs='+', default=[128, 64, 32],
                        help='Hidden dimensions in encoder, e.g., --hidden_dim 128 64')
    parser.add_argument('--lr', type=float, default=1e-3, help='Learning rate')
    parser.add_argument('--batch_size', type=int, default=16, help='Batch size')
    parser.add_argument('--max_epochs', type=int, default=100, help='Maximum epochs')
    parser.add_argument('--patience', type=int, default=100, help='Number of epocs where model training saturated')
...
def main():
    ...
    df_latent.to_csv(f"results/tables/OG_latent_space.csv")
    cv_losses_df = pd.DataFrame(cv_losses, index=[f"fold_{i+1}" for i in range(len(cv_losses))])
    cv_losses_df.to_csv(f"results/tables/OG_cv_losses.csv", index=False, header=False)
...
def each_subtype():
    ...
    for subtype in subtypes:
        ...
        latent = runner.train_all_and_encode()
        df_latent = pd.DataFrame(latent, index=df_exp.index, columns=[f"latent_{i}" for i in range(latent.shape[1])])
        df_latent.to_csv(f"results/tables/OG_latent_space_{LATENT_DIM}dim_{subtype}.csv")
    all_cv_losses_df = pd.DataFrame(all_cv_losses, index=subtypes, columns=[f"fold_{i+1}" for i in range(len(all_cv_losses[0]))])
    all_cv_losses_df.to_csv(f"results/tables/OG_cv_losses_{LATENT_DIM}dim_all_subtypes.csv", index=True)
