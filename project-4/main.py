...
samplingAEParams = {
    'latent_dim': 1000,           # Number of latent features
    'hidden_dims': [625, 750, 875], # Hidden layers
    'sample_size': 800,           # Number of samples to generate
    'lr': 5e-3,                   # Learning rate
    'beta': 0.5,                  # Beta parameter for VAE
    'pretrain_epochs': 100         # Number of epochs to pretrain the VAE
}
...
def main():
    ...
    model, scaler, weights_all = ae_runner.train_all_and_encode(**trainingParams, return_model=True)
    ...
    for subtype in subtypes:
        ...
        df_latent.to_csv(f"results/tables/with_sampling_latent_space_{autoEncoderParams['latent_dim']}dim_{subtype}.csv")
    ...
    torch.save(model.state_dict(), f"results/models/with_sampling_autoencoder_{timestamp}_model_{str_AE_params}.pth")  # save model state

    weights_all = pd.DataFrame(weights_all, index=all_exp.columns)
    weights_all.to_csv(f"results/tables/with_sampling_gene_to_latent_weights_{timestamp}.csv")
    
    cv_losses_df = pd.DataFrame(cv_losses, index=[f"fold_{i+1}" for i in range(len(cv_losses))])
    cv_losses_df.to_csv(f"results/tables/with_sampling_cv_losses_{timestamp}.csv", index=False, header=False)
